		...
		$http.get('/data/latency', {cache: false})
				.success(
								function(data, status, header, config){
										...
										if (data && data.histo.length > 0) {
												min = data.histo[0].x;
												max = data.histo[0].x;
												for (i = 0; i < data.histo.length; ++i) {
													if (data.histo[i].x < min) { min = data.histo[i].x; }
													if (data.histo[i].x > max) { max = data.histo[i].x; }
												}
												//$scope.histOptions.axes.x.min = 0;
												//$scope.histOptions.axes.x.max = 100;
												$scope.histData = data.histo;
										}
								}
						);
}
setInterval($scope.addHistData, 1000);
...
{y: 'y', color: 'steelblue', thickness: '2px', type: 'area', striped: false, label: 'Generated Load'}
		...
		lineMode: 'linear',
		tension: 0.7,
		tooltip: {mode: 'scrubber', formatter: function(x, y, series) {return y + ' kpps';}},
		drawLegend: true,
		drawDots: true,
		columnsHGap: 5
		};
...
$scope.histOptions = {
		axes: {
						x: {key: 'x', labelFunction: function(y) {return y;}, type: 'column', min: 0, max: 100},//max: $scope.data.lenth, ticks: 2},
		y: {type: 'column', min: 0},
},
		series: [
{y: 'y', color: 'green', thickness: '0px', type: 'column', striped: false, label: 'Latency'}
],
		lineMode: 'linear',
		tension: 0.7,
		tooltip: {mode: 'scrubber', formatter: function(x, y, series) {return y +" packets with latency of " + x + "us";}},
		drawLegend: true,
		drawDots: true,
		columnsHGap: 1
		};
